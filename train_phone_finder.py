...
def calculatingaccuracy():
    orgfile = glob.glob(sys.argv[1][2:]+'/*.txt')                        # Append ~/ and read file name from command prompt
...
def main():
    ...
    for im in images:                                                   # Read each image in the folder
        with open(im, 'rb') as file:                                    # Open Image to read
            ...
            filename = im.split('\\')[1]
            plt.imsave ('outputimage/'+ filename , image)               # Drawing image with bounding box
